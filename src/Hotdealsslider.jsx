...
const Hotdealsslider = () => {
  ...
  let [checkprodduct, setcheckprodduct] = useState();
  useEffect(() => {
    const starCountRef = ref(db, "cart-items");
    const unsub = onValue(starCountRef, (snapshot) => {
      const data = snapshot.val() || {};
      setcheckprodduct(data);
    });

    return () => unsub();
  }, []);
  const isavilable = (productname) => {
    return Object.values(checkprodduct).some(
      (item) => item.product === productname
    );
  };

  let handleaddtocart = (item) => {
    if (isavilable(item.name)) {
      toast.error("Already in your cart list");
      return;
    }
    const counref = push(ref(db, "cart-items"));
    set(counref, {
  ...
  return (
    <div className=" container">
      ...
      <div className=" h-[400px] mt-[50px]">
        <h2 className=" text-[23px] font-[500] text-[red] mb-[40px] ml-[5px]">
